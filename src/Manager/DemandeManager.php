...
class DemandeManager
{
    ...
    public function checkServiceClient()
    {
        if ($this->tokenStorage->getToken()->getUser() instanceof User) {
            ...
            foreach($demandes as $demande){
                $tmpStatus = "";
                if ($demande->getTransaction() != null) {
                    $tmpStatus = $demande->getTransaction()->getStatus();
                } elseif ($demande->getCommande() != null && $demande->getCommande()->getTransaction() != null ) {
                    $tmpStatus = $demande->getCommande()->getTransaction()->getStatus();
                }
                if ($tmpStatus == '00') {
                    $status = true;
                    break;
